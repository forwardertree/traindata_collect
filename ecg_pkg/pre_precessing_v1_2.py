...
def pre_process(ecg_, ref_i):
   ...
   '''
    ...
    if ecg_max_val == ecg_min_val:
        ecg_max_val = 1
        ecg_min_val = 0
    ecg_ = 1.5 * (ecg_ - ecg_mean) / (ecg_max_val - ecg_mean)
    ecg_[ecg_ > 2] = 2
    ecg_[ecg_ < -1] = -1
    ...
    if tompk_max_val == tompk_min_val:
        tompk_max_val = 1
        tompk_min_val = 0
    tompk_ = 0.3*(tompk_ - tompk_mean) / (tompk_max_val - tompk_mean)
    tompk_[tompk_ > 2] = 2
    tompk_[tompk_ < -1] = -1
    ...
    # max 조건: 앞, 뒤 10칸씩의 범위 내 최대 and 해당 값 0.7 이상
    if ecg[20 + ref] == max(ecg[10 + ref:30 + ref]) and ecg[20 + ref] > 0.65 and np.sum(
            tompk_max[ref - 3:ref + 3]) > 0 and np.sum(ecg_max) == 0:
        ecg_max[ref] = 1
