...
raw_list = np.array([x for x in os.listdir(raw_path) if 'ecg' in x])
    ...
    for lead_i in range(0, 1):
        ...
        if np.sum(y1) == 0:
            ...
            for raw_i in range(0, len(y1)):
                ...
                tompk_ = (tompk_ - tompk_mean) / (tompk_max_val - tompk_mean)
                tompk_[tompk_ > 2] = 2
                tompk_[tompk_ < -1] = -2
...
with open(r'C:\Users\jjm\Desktop\ecg_ai\trainingDB\ecgdvq_62.5\trainingDB_9_13_240406.pickle', 'wb') as f:
...
# Excel 파일로 저장
excel_path = r'C:\Users\jjm\Desktop\ecg_ai\trainingDB\ecgdvq_62.5\trainingDB_9_13_240406.xlsx'
