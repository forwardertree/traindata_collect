...
raw_list = np.array([x for x in os.listdir(raw_path) if '1ecgrdvq' in x])
    ...
    for lead_i in range(0, 1):
        ...
        if np.sum(y1) == 0:
            ...
            for raw_i in range(0, len(y1)):
                ...
                if tompk[20 + ref] == max(tompk[10 + ref:30 + ref]) and tompk[20 + ref] > 0.65 and np.sum(tompk_max) == 0:
                    tompk_max[ref] = 1

                ecg_max = np.zeros(20)
                ref = np.argmax(ecg[20:40])
                if ecg[20 + ref] == max(ecg[10 + ref:30 + ref]) and ecg[20 + ref] > 0.65 and np.sum(tompk_max[ref-3:ref+3]) > 0 and np.sum(ecg_max) == 0:
                    ecg_max[ref] = 1
...
with open(r'C:\Users\jjm\Desktop\ecg_ai\trainingDB\ecgdvq_62.5\trainingDB_9_13_240326_0.8_1.pickle', 'wb') as f:
...
# Excel 파일로 저장
excel_path = r'C:\Users\jjm\Desktop\ecg_ai\trainingDB\ecgdvq_62.5\trainingDB_9_13_240326_1.xlsx'
