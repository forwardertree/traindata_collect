...
# train_inp을 P, QRS, T 각각 정의했으니까, train_inp -> train_inp_p

with open(r'C:\Users\jjm\Desktop\ecg_ai\trainingDB_9_13_231213.pickle', 'rb') as f:
    trainingdb = pickle.load(f)
train_inp_p = np.array(trainingdb['inp'], dtype='float32')
train_outp_ps = np.array(trainingdb['ps'], dtype='float32')
train_outp_pe = np.array(trainingdb['pe'], dtype='float32')

with open(r'C:\Users\jjm\Desktop\ecg_ai\trainingDB_9_13_231213.pickle', 'rb') as f:
    trainingdb = pickle.load(f)
train_inp_qrs = np.array(trainingdb['inp'], dtype='float32')
train_outp_qrss = np.array(trainingdb['qrss'], dtype='float32')
train_outp_qrse = np.array(trainingdb['qrse'], dtype='float32')

with open(r'C:\Users\jjm\Desktop\ecg_ai\trainingDB_9_13_231213.pickle', 'rb') as f:
    trainingdb = pickle.load(f)
train_inp_t = np.array(trainingdb['inp'], dtype='float32')
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_ps, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_ps, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_ps, keep_prob: 1.0})
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_pe, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_pe, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_pe, keep_prob: 1.0})
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_qrss, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_qrss, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_qrss, keep_prob: 1.0})
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_qrse, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_qrse, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_qrse, keep_prob: 1.0})
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_ts, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_ts, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_ts, keep_prob: 1.0})
...
with tf.Session() as sess:
    ...
    for epoch in range(train_num):
        sess.run(op_train, feed_dict={x: train_inp, y: train_outp_te, keep_prob: drop_out})

        acc_train = sess.run(accuracy, feed_dict={x: train_inp, y: train_outp_te, keep_prob: 1.0})
        cost_train = sess.run(cost, feed_dict={x: train_inp, y: train_outp_te, keep_prob: 1.0})
...
pickle 파일로 저장
'''
save_path = r'K:\ecg_train\231213\ecgrdvq_ai_model_231213.pickle'
...
acc, cost 결과 저장
'''
np.savetxt(r'K:\ecg_train\231213\dnn_result_acc_231213.csv', acc_cost_arr, fmt='%s', delimiter=",")
